        ...
        <div className="basketList">
          <List sx={{ width: "100%", bgcolor: "background.paper" }}>
            {Object.values(uniqueItems).map(({ item, count }) => (
              <ListItem key={item} disableGutters className="basketListItem">
                <BasketCard basketItem={item} basketProductCount={count} />
              </ListItem>
            ))}
          </List>
        </div>
        <div className="costPrice">
          <Card>
            ...
            <List className="orderSummarylist">
              {Object.values(uniqueItems).map(({ item, count }) => (
                <ListItem key={item} disableGutters className="costPriceItem">
              ...
              <ListItem>
                <Typography className="totalPrice">
                  ...
                  <div>
                    ...
                    {Object.values(uniqueItems).reduce(
                      (total, { item, count }) => total + count * item.price,
                      0,
                    )}
                  </div>
                </Typography>
